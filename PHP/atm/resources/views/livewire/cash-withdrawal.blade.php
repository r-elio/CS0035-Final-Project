<div class="container px-5">
            ...
            <form wire:submit.prevent="submit">
                ...
                <div class="d-flex justify-content-center mt-5">
                    <button class="btn btn-outline-light btn-lg me-4"
                            type="submit">{{ __('messages.withdraw') }}</button>
    ...
    @if(session('invalid_withdraw'))
        <script>
            $(document).ready(function () {
                $('#{{ session('invalid_withdraw') }}').modal('show');
            });
        </script>

        <div class="modal fade" id="invalid_withdraw" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content text-center">
                    <div class="modal-header">
                        <h5 class="modal-title">{{ __('messages.invalid withdraw amount') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" style="white-space: pre-line">{{ __('messages.invalid withdraw amount content') }}</div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">OK</button>
                    </div>
                </div>
